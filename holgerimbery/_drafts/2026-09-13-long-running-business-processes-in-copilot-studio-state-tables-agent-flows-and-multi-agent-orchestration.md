...
*How I build long-running processes, like invoice processing or purchase order handling, with the standard harness — no custom middleware, no side platform.*
...
Nothing in that story fits into a chat turn. But all of it fits into a **state table**.

My way of working is simple: the agent is not the process. The agent is a worker that picks up work, does one well-scoped thing, writes down what it did, and moves the item to the next state. The process lives in Dataverse. That single decision is what made long-running automation reliable for me — and everything I describe here I build with the standard harness: Copilot Studio, agent flows, Dataverse, connectors. No custom orchestration layer, no separate runtime to operate.
...
| `AssignedAgent` | Which specialised agent currently owns the item |
...
On top of the table sits one orchestrator. It is a Copilot Studio agent with a deliberately thin job description: read the case, decide which specialist should handle the current state, hand over, and write the result back. It has no domain knowledge about VAT rules or supplier master data. It knows states and routing.
...
- **Scheduled** — a recurring agent flow that sweeps due cases. This is the heartbeat that makes "long-running" work at all. A process that waits four days does not need a session that stays open for four days; it needs a table row with a date and a sweeper.
...
The specialist agents are where the domain knowledge lives, and each one is scoped small enough that I can describe its job in one sentence.
...
**Scanned-document extraction agent.** This is the one people ask about most. Scanned PDFs and photographed invoices are a different problem from digital PDFs, and I treat them as such. The agent works through an agent flow that runs the document through OCR / document intelligence, then reasons over the recognised content to produce the structured payload: supplier, invoice number, invoice date, net, tax, gross, currency, PO number, line items, IBAN. It returns a per-field confidence, not just values — and I persist that confidence, because it is the switch that decides whether a human ever sees the document.

**Digital PDF extraction agent.** Same output contract, different path: text layer first, OCR only as fallback. Same JSON schema so that everything downstream is indifferent to how the data was obtained. Agreeing one schema across all extraction paths is the single highest-leverage design decision in the whole solution.

**Validator.** Arithmetic, tax plausibility, IBAN format, duplicate detection against previously posted invoices, supplier existence in master data. Deterministic checks belong in flows; judgement calls belong in the agent. I keep that line clean.
...
**Approval agent.** Determines the approver from amount, cost centre and delegation rules, sends an adaptive card, and waits. "Waits" here means: writes `PendingApproval`, sets `NextActionAt` for the reminder, and stops. Nothing stays in memory.

**Posting agent.** Talks to the ERP through a connector, with the write-before/confirm-after pattern.

**Communication and alerting agent.** Manages SLAs, deadlines, and escalations. When a case breaches its SLA target or an approval sits unattended, it fires reminders via Teams or email, escalates to backup approvers, and records every notification back to the state table.

**Supplier inquiry agent.** Triggers an email back to the invoice sender whenever essential data is missing or invalid — like an absent PO number, unverifiable tax ID, or mismatching line items. It specifies the exact issue, attaches the reference, transitions the case to `WaitingForSupplier`, and sets a deadline so the process parks cleanly until a reply arrives.

Each specialist gets the case ID and reads what it needs from the table. I deliberately do not pass large context between agents. The state table is the shared memory, and it is queryable, auditable and survives restarts — three things a conversation context does not.

## What the state table gives me beyond orchestration

Once the process lives in Dataverse, a lot of things I used to build separately come for free.
...
**Safe evolution.** Adding a new step to the process means adding a state and a specialist. Nothing else changes, because nothing else knew the shape of the process to begin with.
...
- Put deterministic logic in agent flows, judgement in agents, and truth in Dataverse.
...
The technology stack here is unglamorous on purpose. The interesting engineering is not in the agent — it is in the table that remembers what the agent did.
