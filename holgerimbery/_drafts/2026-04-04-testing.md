...
```mermaid
flowchart LR
    A[Agent Design & Authoring<br/>Copilot Studio] --> B

    subgraph B["Agent Evaluation (GA)"]
        B1[AI-generated & manual test queries]
        B2[Semantic similarity<br/>Relevance & completeness]
        B3[Intent & behavior validation]
    end

    B --> C

    subgraph C["Copilot Studio Kit (Power CAT)"]
        C1[Deterministic tests<br/>Response / Topic / Attachments]
        C2[Multi-turn end-to-end scenarios]
        C3[Generative orchestration<br/>Plan & tool validation]
        C4[Telemetry, KPIs, CI/CD gates]
    end

    C --> D[Production Deployment<br/>Governance & Compliance]
```
