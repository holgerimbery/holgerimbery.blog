...
## TThe testing toolbox at a glance
...
Agent evaluation is a built‑in [(preview)](https://learn.microsoft.com/en-us/microsoft-copilot-studio/analytics-agent-evaluation-intro) feature of Copilot Studio that lets you author or generate test sets and run automated evaluations directly in the product. It is designed to measure answer quality and coverage at scale and can generate tests from your agent’s topics/knowledge or import them from a file. Evaluations execute with a designated test user profile, which is essential when tools and knowledge sources require authentication. Because this feature is in preview, treat it as a complementary capability to the Kit: use it for fast, in‑product evaluations and keep the Kit’s runs and exports for long‑term retention and pipeline gating.   
**When to use**:
...
Operations
 ...
 - Compliance Hub monitoring and triage workflows active
 - CoE telemetry and inventory integrated for ongoing oversight


### Why Phase 4 matters
Phase 4 formalizes the last mile from “it works in development” to “it is safe to expose to users.” It replaces manual spot‑checks and one‑click publishing with automated validation, governed deployment, and observable operations—the baseline expected for enterprise‑grade AI agents. 
...
If your agent uses Microsoft Authentication, set up an app registration and the Kit’s test automation configuration so the test runner can authenticate like a user.

### Design test cases and test sets
Supported test types (Kit):

* Response match with string operators (equals/contains/starts/ends).
...
Run test sets against your agent; Kit records observed responses, latencies, and aggregates. Enrichment adds topic routing and detailed diagnostics. Export results (CSV) for long‑term retention or integration with other tools.
...
Clicking Publish pushes current agent state to channels; it is not a deployment pipeline. Without environments, solutions, and quality gates, you lack versioning, rollback, and documented test evidence—unacceptable for enterprise operations. Use solutions + pipelines + automated tests instead.
...
### Performance
Execute load tests through Direct Line, prefer WebSockets, track the specific latencies Microsoft recommends.   
