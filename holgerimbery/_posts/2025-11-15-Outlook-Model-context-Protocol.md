...
MCP supports event-driven workflows:
    ...
    - Trigger based on conditions (e.g., subject contains “Invoice”)
...
MCP allows multiple agents to share the same Outlook context without duplicating configuration, enabling enterprise-scale automation.
...
Scenario
Create an autonomous agent that monitors a shared mailbox for order status inquiries. When an email is received requesting order status information, the agent:
...
- Extract sender, subject, and attachment metadata from MCP context.
...
- Send a test email with an invoice attachment to the monitored mailbox.
