...
**How to tell which one a document means:** a SKILL.md, YAML front matter, or a ZIP package means the new model. A **skill manifest**, an allow list, an **Azure Bot Service** bot, or "classic chatbot" means the legacy model — and it doesn't apply to the agents here. Throughout this article, ** skills ** are the new Markdown format.
...
Reuse has pros and cons: one Skill can serve many agents, but a bad Skill can spread problems just as widely. Governance needs to keep up with this:
