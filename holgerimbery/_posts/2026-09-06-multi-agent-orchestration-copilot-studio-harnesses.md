...
{: .q-left }
> The last article was about workflows and the agent node. This one goes one level up. Copilot Studio now runs three harnesses, and the harness you pick decides which multi-agent building blocks you are even allowed to use. Child agents, connected agents, A2A and MCP are not four flavours of the same thing, and they are not all available everywhere.
...
3. **One agent, child agents** (standard harness). An embedded specialist that needs its own tool budget, its own typed inputs, its own trigger.
...
Steps 2 and 3 are in-agent complexity. Steps 4 and 5 are cross-agent work. Conflating those two categories is the single most expensive modelling mistake I see, and it produces either an unmaintainable god-agent or an org chart of agents nobody can trace a request through.
...
**Priority and ordering.** "A lower number indicates a higher priority", ties resolved "in the order of creation (oldest first)". Child agents and topics share the same trigger set, so priority is how you keep them from fighting.

**Post-completion behaviour.** After running: don't respond (the default), write the response with generative AI, send a specific response, or send an adaptive card.
...
## Skills: reusable behaviour, not a reusable agent
...
Where a skill wins: the behaviour belongs to one agent, the capability is essentially instructions plus transformation logic plus templates, you want to attach it to several agents, and you do not need an independently governed identity.
...
- **Skill** — behaviour belonging inside one agent.
...
In-agent complexity becomes skills. Cross-agent work stays connected agents. Everything that leaves the tenant boundary is a tool or A2A.
...
Standard-harness agents are the responsive, controlled front door. Workflows own the approvals, the writes and the system updates, because that is where determinism is provable. GitHub-harness agents take the heavy analysis, the file processing, the exception resolution, the curation. Skills encode the operating procedures. Connected agents represent independently owned domain specialists. Everything that touches an external system goes through a governed tool.
...
- **Do you need an external agent?** If yes, it is standard harness today. A2A is not documented in the new harness.
- **Is that thing a skill or a connection?** If it authenticates or leaves the sandbox, it is a tool. Always.
- **Have you refactored network-dependent skill code?** `pip install`, direct API calls and database queries do not survive the move.
- **Where does the generated file go?** The sandbox is temporary, files cap at 10 MB and expire after 28 days. Persist through a tool or hand it to the user.
...
- **Did you confirm the preview status yourself?** Connected agents, memory, Work IQ and user file upload are marked preview in the blog; some Learn pages are still prerelease.
