...
- **Power Apps per App is End of Sale (January 2026).** Per the PPLG change log, the per-App **subscription license** reached End of Sale; only the **per-App pay-as-you-go meter ($10 / active user / app / month)** remains. §1.1 and the per-App status flag are updated (the Rev 2 "reinstated / to-verify" note is superseded).
- **Dataverse tenant defaults (§3.1).** The PPLG confirms **File default 20 GB and Log default 2 GB**; its worked example uses a **10 GB Database default** while its capacity table lists 20 GB — an internal inconsistency in the guide itself. Defaults were increased in November 2025.
- **AI Builder (§2.2.1).** Corrected to **End of Sale November 2025**, with existing entitlements available until **November 1, 2026** (PPLG change log).
...
- **New "build harness" concept (§2.0).** The guide introduces **three harnesses** — **GitHub Copilot** (agentic build; consumes credits at *both* maker/build time and runtime), **Standard** (rule-based topics/flows), and **Copilot Chat** (customize Microsoft 365 Copilot). A new §2.0 explains them and their credit-consumption implications.
- **Dataverse-for-Copilot-Studio default capacity raised (§3.1).** Per the guide, the Copilot Studio Dataverse default is now **15 GB Database / 20 GB File / 2 GB Log** per tenant — the Database default rose from 5 GB to 15 GB in December 2025. Noted in §3.1 as distinct from the general Power Platform tenant default.
- **Dataverse add-ons confirmed and expanded (§3.3).** The guide confirms the subscription add-on list prices ($40 DB, $2 File, $10 Log per GB/month, billed annually) and adds a **Database Tier 2 add-on at $30/GB/month (1,000 GB minimum)**. It also documents **pay-as-you-go Dataverse capacity meters** (per environment: **$48/GB DB, $2.40/GB File, $12/GB Log per month**, above a 1 GB DB + 1 GB File per-environment entitlement, no Log). §3.3 updated.
- **Work IQ APIs (§2.3).** Per the guide's July 2026 note, Copilot Studio agents' use of **Work IQ APIs is not included** in the Microsoft 365 Copilot USL and consumes credits. Noted in §2.3.
...
(a) per-user Dataverse accrual figures for **Dynamics 365** SKUs from the current Dynamics 365 Licensing Guide (the Power Platform figures are now confirmed against the August 2026 PPLG — Power Apps Premium accrues 250 MB DB / 2 GB File per user, and the larger numbers are tenant defaults, not per-user);  
...
Sources: [Power Automate pricing](https://www.microsoft.com/en-us/power-platform/products/power-automate/pricing), [Power Platform Licensing Guide August 2026](https://go.microsoft.com/fwlink/?linkid=2085130). Power Automate Premium ($15/user/month) and Process ($150/bot/month) are confirmed in the August 2026 guide.
...
### 2.0 Build harnesses (new in the August 2026 guide)

The August 2026 Copilot Studio Licensing Guide introduces **three "harnesses"** — the software scaffolding an agent uses to plan, reason, keep context, and call tools. The harness a maker chooses affects **when** Copilot Credits are consumed:

| Harness | What it's for | Credit consumption |
|---------|---------------|--------------------|
| **GitHub Copilot harness** | Optimize end-to-end business processes with an agentic build experience | Consumes Copilot Credits at **both build/maker time and runtime** |
| **Standard harness** | Rule-based conversational agents with predefined topics, flows, and workflows | LLM-powered maker features consume credits; runtime billed via Copilot Credits (fair-usage limits apply in Microsoft channels) |
| **Copilot Chat harness** | Customize Microsoft 365 Copilot with your organization's knowledge | LLM-powered maker features consume credits; runtime billed via Copilot Credits (fair-usage limits apply in Microsoft channels) |

Manual, non-LLM configuration does **not** consume credits; natural-language authoring, Evaluation, and Preview do. For Microsoft 365 Copilot USL holders, employee-facing (B2E) runtime of **Standard** and **Copilot Chat** harness agents in Microsoft channels is included in the USL and does not require a Copilot Credit purchasing option to be configured; **GitHub Copilot** harness runtime always consumes credits. (Source: Copilot Studio Licensing Guide, August 2026.)

### 2.1 Commercial models

The August 2026 guide defines **four purchasing constructs** for Copilot Credits, plus the zero-rated M365 Copilot path:

| Purchasing construct | List price (USD) | How it works |
|----------------------|------------------|--------------|
| **1. Pay-as-you-go Copilot Credit** (Azure-billed meter) | **$0.01 / Copilot Credit** | Post-pay for the exact credits agents and flows consume in a billing month; no upfront commitment. Decrements the Microsoft Azure Consumption Commitment (MACC); not Azure-prepayment eligible. |
| **2. Copilot Credit Pre-Purchase Plan (P3)** | Tiered discount off PAYG (see tiers below) | One-year pay-upfront pool of Copilot Credits that auto-decrements; overage rolls to PAYG or an added pool. Unused credits expire at the end of the annual term. |
| **3. Microsoft Agent Pre-Purchase Plan (P3)** | Tiered discount (see tiers below) | One-year pay-upfront pool priced in **Agent Commit Units (ACUs)** that covers eligible usage across **Copilot Studio *and* Microsoft Foundry**. **1 ACU = $1 = 100 Copilot Credits.** Per environment via billing plan; unused ACUs do not roll over. |
| **4. Copilot Studio capacity pack** (subscription license) | **$200 / pack / month, billed annually = 25,000 Copilot Credits** (≈ $0.008 / credit) | Per-tenant subscription; buy packs for expected monthly volume. Unused credits do not roll over month to month. Each builder also needs a **Copilot Studio User License ($0)**. |
| **M365 Copilot included usage** (not a purchase) | Zero-rated for licensed M365 Copilot users | B2E runtime of Standard / Copilot Chat harness agents in Microsoft channels is included in the $30/user/month M365 Copilot USL when the agent runs under the licensed user's identity; no tenant credits drawn. |

**Copilot Credit Pre-Purchase Plan (P3) tiers** (one-year, pay-upfront):
...
**Microsoft Agent Pre-Purchase Plan (P3) tiers** (one-year, pay-upfront; priced in ACUs, 1 ACU = $1 = 100 Copilot Credits):

| Tier | Agent Commit Units (ACUs) | Discount |
|------|---------------------------|----------|
| 1 | 20,000 | 5% |
| 2 | 100,000 | 10% |
| 3 | 500,000 | 15% |

Sources: Microsoft Copilot Studio Licensing Guide — August 2026; [Copilot Studio pricing](https://www.microsoft.com/en-us/microsoft-copilot/microsoft-copilot-studio/pricing); [Microsoft Agent Prepurchase Plan (Microsoft Learn)](https://learn.microsoft.com/en-us/azure/cost-management-billing/reservations/agent-pre-purchase); [Copilot Credits pre-purchase](https://learn.microsoft.com/en-us/microsoft-copilot-studio/requirements-pre-purchase-pricing); [M365 Copilot included entitlement](https://learn.microsoft.com/en-us/microsoft-copilot-studio/requirements-licensing-microsoft-copilot-included).
...
| **Tenant Microsoft Graph grounding** (employee context) | **10** (reduced from 30; confirmed in the Power Platform Licensing Guide change log, April 2025) |
| **Voice** (per minute) | **13** |
| **Autonomous action** (event-triggered, no user prompt) | **25** |

{: .warning }
**Confidence note.** Microsoft rebranded "messages" to "Copilot Credits" on September 1, 2025; per its Copilot Studio licensing documentation the pay-as-you-go rate and the prepaid-pack quantity did **not** change with the rename, but per-event consumption rates should be treated as unverified. The table above reflects the rates reported in the April 2026 licensing guide and has not been independently re-verified as of August 4, 2026. The **August 2026 Copilot Studio Licensing Guide still lists per-event rates only by reference to the separate [Billing rates and management](https://learn.microsoft.com/en-us/microsoft-copilot-studio/requirements-messages-capacity) page**, which remains authoritative — confirm before sizing a deployment.
...
AI Builder reached **End of Sale in November 2025** per the Power Platform Licensing Guide change log; existing **AI Builder entitlements for current Power Apps, Power Automate, and Dynamics 365 customers remain available until November 1, 2026**, after which the capability consolidates under the Copilot Credit meter. It serves form processing / document automation, prediction models, sentiment analysis, and category classification scenarios — distinct from Copilot Studio agent invocations. Customers with active AI Builder entitlements can use them through that date; net-new deployments after it will provision Copilot Credits instead.

**During the transition (now through November 2026):** if a workload is predominantly document processing, AI Builder may still be the more economical SKU. If it mixes document processing with conversational agents, Copilot Credits are the forward-compatible path. Re-check the current Copilot Studio Licensing Guide before committing to either ([Copilot Studio Licensing Guide August 2026 (PDF)](https://go.microsoft.com/fwlink/?linkid=2243406)).
...
{: .note }
**Work IQ APIs are not zero-rated.** Per the August 2026 guide (a July 2026 change), a Copilot Studio agent's use of **Work IQ APIs is not included** in the Microsoft 365 Copilot USL and consumes Copilot Credits even in otherwise-included B2E scenarios.
...
- Capacity packs: 5 × $200 packs = **$1,000 / month** for 125,000 credits (5 × 25,000) — the effective $0.008 / credit is exactly a 20% saving vs PAYG.

**Rule of thumb:** Above ~25,000 credits / month, the **$200 capacity pack** beats PAYG. For large annual commitments the **Copilot Credit Pre-Purchase Plan (P3)** layers on a further 5–20% tiered discount, and the **Microsoft Agent Pre-Purchase Plan (P3)** is the option when the same up-front commitment must also cover Microsoft Foundry usage.
...
The first eligible subscription provisioned in a tenant grants a **one-time tenant-level default**. The **August 2026 Power Platform Licensing Guide** puts the **File default at 20 GB and the Log default at 2 GB**; for the **Database default** the guide is internally inconsistent — its worked example uses **10 GB** while its capacity table lists **20 GB** (these tenant defaults were raised in November 2025). This is not per subscription — purchasing a second Power Apps Premium or Dynamics 365 SKU does NOT double the default. The per-environment default (3 GB DB / 3 GB File / 1 GB Log for the default environment) is separate. Verify both figures against the current licensing guide ([Dataverse capacity storage](https://learn.microsoft.com/en-us/power-platform/admin/capacity-storage)). The **August 2026 Copilot Studio Licensing Guide** states a **Dataverse-for-Copilot-Studio default of 15 GB Database / 20 GB File / 2 GB Log** per tenant (the Database default rose from 5 GB to 15 GB in December 2025) — the entitlement a tenant's first Copilot Studio / Power Apps / Power Automate / Power Pages or select Dynamics 365 subscription unlocks. Do not confuse it with the general Power Platform tenant default above.

### 3.2 Per-license accrued capacity

Dataverse capacity has two distinct dimensions that are easy to confuse: a **one-time tenant Default** (raised in November 2025) and **per-user Accrued** capacity (added for each license). The August 2026 Power Platform Licensing Guide lists them in separate columns, and its per-user numbers agree with the live pricing pages. The table below shows the **per-user accrued** values; the tenant default is covered in §3.1.

| License | DB accrued (live pricing page) | File accrued (live pricing page) | Licensing Guide PDF reports |
|---------|-------------------------------|----------------------------------|-----------------------------|
| Power Apps Premium | 250 MB / user | 2 GB / user | **Confirmed** by the August 2026 PPLG (Accrued column). The 20 GB sometimes cited is the tenant-wide **Default**, not per-user accrual — see §3.1. |
...
Sources: [Power Apps pricing](https://www.microsoft.com/en-us/power-platform/products/power-apps/pricing); [Power Automate pricing](https://www.microsoft.com/en-us/power-platform/products/power-automate/pricing); [Power Platform Licensing Guide August 2026 (PDF)](https://go.microsoft.com/fwlink/?linkid=2085130); [Dynamics 365 Licensing Deck March 2026 (PDF)](https://cdn-dynmedia-1.microsoft.com/is/content/microsoftcorp/microsoft/bade/documents/products-and-services/en-us/bizapps/Dynamics-365-Licensing-Deck-March-2026-PUB.pdf).

{: .important }
**Default vs per-user accrual — resolved for Power Platform.** The August 2026 Power Platform Licensing Guide confirms Power Apps Premium accrues **250 MB DB + 2 GB File per user** — the same as the pricing page. The larger numbers that circulated (e.g., 20 GB) are the **one-time tenant Database default**, not a per-user accrual, and must not be multiplied per user. For **Dynamics 365** SKUs the analogous per-user figures still come from the Dynamics 365 Licensing Guide (see §3.5) — verify those the same way before sizing.

### 3.3 Capacity add-ons

**Subscription capacity add-ons** (billed annually; confirmed in the August 2026 Copilot Studio Licensing Guide):

| Add-on | Minimum quantity | List price (USD) |
|--------|------------------|------------------|
| Dataverse Database capacity | 1 GB | **$40 / GB / month** |
| Dataverse Database capacity **Tier 2** | 1,000 GB | **$30 / GB / month** |
| Dataverse File capacity | 1 GB | **$2 / GB / month** |
| Dataverse Log capacity | 1 GB | **$10 / GB / month** |

**Pay-as-you-go Dataverse capacity meters** (per environment, billed on consumption above a 1 GB Database + 1 GB File per-environment entitlement — no Log entitlement):

| Meter | List price (USD) |
|-------|------------------|
| Dataverse Database capacity | **$48 / GB / month** |
| Dataverse File capacity | **$2.40 / GB / month** |
| Dataverse Log capacity | **$12 / GB / month** |

Subscription capacity is purchased per tenant and allocated across environments by an admin in the Power Platform admin center; the pay-as-you-go meters bill per environment as an alternative to subscription add-ons for overages. Dataverse capacity add-ons do not apply to Dataverse-for-Teams environments.
...
A 200-user Dynamics 365 Customer Service Enterprise tenant in 2026. **Caution:** the 30 GB DB / 30 GB File "per Enterprise CRM user" figure used below comes from an earlier Dynamics 365 Licensing Guide reading and may repeat the default-vs-accrued confusion corrected for Power Apps in §3.2 — re-verify per-user *accrued* capacity against the current Dynamics 365 Licensing Guide before relying on it. Using that figure:
...
The structurally interesting shift of the last twelve months is the consolidation of Power Platform AI consumption under a single **Copilot Credit** meter (with AI Builder credits scheduled to be retired in November 2026) — a strategic move toward a unified Azure-billed currency for all AI workloads. Future licensing decisions will increasingly be about *how much AI you can commit to up front* (capacity packs and the Copilot Credit or cross-platform Microsoft Agent Pre-Purchase Plans) versus *flexibility to spike* (pay-as-you-go), rather than which SKU contains which feature. In parallel, the August 2026 Power Platform Licensing Guide clarified what had looked like a pricing-page-vs-guide discrepancy: it is the **one-time tenant Dataverse default** that rose (November 2025), not per-user accrual — Power Apps Premium still accrues 250 MB DB / 2 GB File per user. Dynamics 365 per-user figures should still be re-verified against the current Dynamics 365 Licensing Guide at contract signing.

For the practitioner, three defaults: **Power Apps Premium for new build-outs unless app usage is clearly segmented by role; Customer Service Premium over Enterprise + add-ins for any deployment that includes both voice and digital messaging (the 40% Contact Center promo that made this especially attractive ran to June 30, 2026 and has since closed — Enterprise was never eligible); Copilot Studio capacity packs above ~25,000 credits / month, stepping up to the Copilot Credit or Microsoft Agent Pre-Purchase Plan (P3) for large annual commitments.** Confidence in these recommendations is HIGH for prices on Microsoft's own pricing pages and MODERATE for add-in list prices, Copilot Credit billing rates, and Licensing Guide PDF Dataverse accrual figures — all should be re-verified before any commercial commitment.

This article is revised on a rolling basis. The **Changelog** at the top records every license-relevant change Microsoft publishes — price adjustments, SKU additions or retirements, capacity changes, billing rate revisions — with effective date, source, and a one-line impact note.
...
1. [Microsoft Power Platform Licensing Guide — August 2026 (PDF)](https://go.microsoft.com/fwlink/?linkid=2085130)
...
4. [Microsoft Copilot Studio Licensing Guide (fwlink → current edition; **August 2026** at the time of writing)](https://go.microsoft.com/fwlink/?linkid=2243406)
