...
{: .q-left }
**None of this is a workaround** for a missing feature: Microsoft's own approvals guidance tells you to store approvals in Dataverse and split the work across two flows the moment a process might run longer than 30 days, because a cloud flow's run duration is capped there and any pending steps simply time out when it expires — and that ceiling is really two, since the run history is discarded at the same point and takes the audit trail with it. Everyone who builds durable workflow engines already solved this the same way: Temporal persists its event history to a database, Durable Functions checkpoints to a storage provider, and Conductor keeps its workers stateless so executions survive a restart, all of them externalising state and rehydrating — which is exactly what a case row and a sweeper do, with the persistence layer in plain sight instead of hidden behind a runtime. In Copilot Studio the point settles itself, because a conversation times out after 30 minutes of inactivity, so for an invoice that lives six weeks the row has to be the memory. **The orchestrator is a different question**, and the honest answer is that if your routing is a genuine lookup, build it as a flow — a flow is deterministic, the same input always produces the same output, and a switch statement costs nothing; the agent earns its place only where routing needs judgement rather than a match, and encoding those judgement calls as branches is how a flow grows toward the documented 500-action, eight-level-nesting wall. So I let the agent decide and never act: every write, every sweep and every ERP call is a flow, the transition is guarded, and a wrong decision gets refused at the write instead of halfway through posting an invoice
